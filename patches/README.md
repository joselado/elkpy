...
| [0020](0020-density-postprocessing.patch) | — | `src/elkpy_eigenstates.f90` (elkpy's own file, added by 0003) | A further block at the end of 0019's `elkpy_denskexport`, calling upstream `rfshtip`, `rfmtctof` and `rfinterp` on local arrays | The two steps between `rhomagk`'s output and the density the potential is built on, as **references and as an operator**. References: the muffin-tin density after `rhomagsh` (coarse mesh, back in harmonics) and after `rfmtctof` (fine radial mesh), so each step is checkable alone rather than through their composition. Operator: `rfmtctof` is a fixed linear map — `rfinterp`'s spline weights come from `wspline` and depend only on the mesh — so it is exported as a matrix built by calling `rfinterp` on basis vectors, the same call as 0018. **Two** per species, because `rfmtctof` interpolates the whole radial range for $\ell\le l_{\max}^{\rm i}$ and the outer region alone above it. With `symtype=0` these two steps plus 0019's complete `rhomagv` for a non-magnetic cell; `src/elkjax/density.py` reproduces all three stages to 1e-15 (`docs/jax_port_phase2.md` §2h) |
| [0021](0021-core-density-and-charge.patch) | — | `src/elkpy_eigenstates.f90` (elkpy's own file, added by 0003) | A final block at the end of `elkpy_denskexport`; no new call | `rhocr` (the core density, which `rhocore` adds to the $\ell=0$ slot with the $1/y_{00}$ already folded in, the same convention as `vcln`), `nspncr`, and `chgtot` — the electron count `rhonorm` normalises to. The core states are a functional of the potential, so at fixed potential the core density is an INPUT exactly as `vsmt` is; exporting it is the same call as 0015's, not a step declined. With it `src/elkjax/density.py` reproduces Elk's own converged `rhomt`/`rhoir` — not an intermediate — at 3e-13 and 6e-12 (`docs/jax_port_phase2.md` §2h) |
| [0022](0022-interstitial-symmetrisation.patch) | — | `src/elkpy_eigenstates.f90` (elkpy's own file, added by 0003) | A further block in `elkpy_denskexport`; no new call | `symrfir` as a **G-space map and phase**: for each of the `nsymcrys` operations and each of the `ngvc` G-vectors, the index it is drawn from and the translation phase. `symrfir` symmetrises a real interstitial function by averaging $\rho(S\mathbf G)e^{-i(S\mathbf G)\cdot\mathbf t}$ — a permutation plus a phase — so the whole operator is two small arrays where a real-space form would be `ngtc`-square. Exported for 0018's reason: reproducing the map means transcribing `symlat`/`lsplsymc`/`vtcsymc`/`ivgig` and the rotation's transpose convention, none of which has an independent check. Together with 0018's muffin-tin operator this **lifts the `symtype=0` restriction**: on Elk's default reduced mesh the density goes from 0.165 to 1.1e-15 (interstitial) and 8e-6 to 1.9e-13 (muffin tin) |
| [0023](0023-occupations-export.patch) | — | `src/elkpy_eigenstates.f90` (elkpy's own file, added by 0003) | A final block in `elkpy_denskexport`; no new call | `occupy.f90`'s inputs and its own answer: `stype`, `swidth`, `occmax`, `chgval`, `e0min`, `epsocc`, Elk's converged `efermi`, `evalsv` over the WHOLE k-set, and the core half of `evalsum` (`sum occcr*evalcr`). The Fermi level is a zone sum, so no single-k query can supply its inputs — `LAPW` returns `evalfv` at one k-point and `DENSITYK` wrote only the occupations. With `evalsv` here, `src/elkjax/occupations.py` is checked against Elk's own `efermi`/`occsv` on Elk's own eigenvalues, i.e. with the assembly OUT of the path: a wrong bisection and a wrong Hamiltonian are otherwise the same symptom. Measured, both are reproduced bitwise on bulk Si and fcc Al. `e0min` is exported rather than derived because it is `min(0, every apwe0 and lorbe0) - 2` and the linearisation energies patch 0015 exports are per-l and post-search; the core half of `evalsum` for the same reason patch 0021 exports `rhocr` — the core solver is an input at fixed potential, not a step declined (`docs/jax_port_phase3.md`) |
| [0024](0024-initial-state.patch) | `src/elkpy_initstate.f90` | `src/elk.f90` (task dispatch `case`), `src/Makefile` (`SRC_ELKPY` var), `src/elkpy_eigenstates.f90` (elkpy's own file, added by 0003) | `elk.f90`: one new `case` arm (9006); a further block of `write` statements at the end of 0013's `elkpy_lapwexport` and one more at the end of 0019's `elkpy_denskexport`, which now calls upstream `energykncr` | **Task 9006 -- the ground state at iteration ZERO.** Every export in 0013-0023 is reached through task 1, which reads a converged `STATE.OUT`, so the JAX port could only ever be started from Elk's own answer -- and Elk's own answer is a fixed point of the port's map, so the loop had nothing to do (`docs/jax_port_phase3.md` §3b perturbed it *by hand*). This task is `gndstate.f90`'s own `trdstate=.false.` branch -- `init0`, `init1`, `rhoinit`, `potks(.true.)`, `genvsig` -- followed by the top of its first self-consistent iteration -- `gencore`, `linengy`, `genapwlofr`, `gensocfr`, `genevfsv`, `occupy` -- and **nothing after it**: no `rhomag`, no `potks` on a new density, no `mixerifc`. Every array the 9002 queries read then holds its iteration-zero value, with the density the superposition of free atomic densities `rhoinit` builds, and `elkjax.driver` iterates from there to self-consistency in JAX (`docs/jax_port_phase3.md` §3c). Nothing here is new physics: every line is an upstream call in `gndstate`'s own order. The `LAPW` addition is the `autolinengy` flag and the per-orbital `apwve`/`lorbve` flags, which say whether `linengy` re-searches a linearisation energy each iteration or leaves it at the species file's default -- the port freezes them, which is EXACT in the second case and an approximation in the first, and `apwe` alone cannot tell the two apart. The `DENSITYK` addition is **`engykncr`**, the core KINETIC energy at the exported potential. `energy.f90` builds the kinetic energy as $\Sigma_\varepsilon-\int\rho v_{cl}-\int\rho v_{xc}$, so the core's share of it is `evalsumcr` MINUS $\int\rho_{\rm core}v_s$, with both halves at the current potential. A loop that freezes the core must freeze *that difference*, not `evalsumcr` alone -- otherwise the same core density is integrated against a potential its eigenvalues never saw, a first-order error over 20 core electrons sitting where the potential moves most. Measured on bulk Si from an atomic-superposition start: **2.03 Ha**, against a true core relaxation of 1.5e-3 Ha. Exported so the transcription of $\int\rho_{\rm core}v_s$ can be pinned -- it is exactly `evalsumcr - engykncr` -- which it is, to 5e-16 at both the initial and the converged export |
| [0025](0025-tunnelling-fermi-surface.patch) | `src/elkpy_fermitunnel.f90` | `src/elk.f90` (task dispatch `case` + docs), `src/modmain.f90` (new module vars), `src/readinput.f90` (`elkpy_fermitunnel_planes`/`_window`/`_kgrid`/`_koffset`/`_sdir`/`_spol`/`_tdir`/`_tpol` block parsers), `src/Makefile` (`SRC_ELKPY` var) | `elk.f90`: one new `case` arm (9007) | Task 9007 -- the tunnelling Fermi surface seen by a junction whose tip is an infinite PLANE. Where 0012's task 9005 sums a point tip's amplitudes over the exit plane, this one puts a Gram matrix on BOTH sides: $W(\mathbf k;E)=\mathrm{Tr}[D\,G^{\rm t}_{\mathbf k}\,D\,S_{\mathbf k}]$ with $D=\mathrm{diag}\sqrt{\delta_\eta(E-\varepsilon_{n\mathbf k})}$, $S_{\mathbf k}$ the substrate-plane Gram matrix 0012 already builds and $G^{\rm t}_{\mathbf k}$ the same object on the tip plane -- so it is the $\mathbf k$-decomposition of 0012's map, not a new approximation: $\int_{\rm cell}T(\mathbf r;E)\,d^2r=\sum_{\mathbf k}w_{\mathbf k}W(\mathbf k;E)$ holds exactly, and that identity is how the task is verified. Setting both Gram matrices to the identity returns the plain Fermi surface $\sum_n\delta_\eta(E-\varepsilon_{n\mathbf k})$ on the same k-mesh from the same run, so the ratio is the junction's own transfer function. **The vacuum weighting has an `rgkmax` floor and the large-$|\mathbf k|$ pocket hits it first** -- see `docs/design.md` §35 before quoting any contrast ratio |
| [0026](0026-bandstr-elm-real8.patch) | — | `src/bandstr.f90` | One declaration line split in two | **A fix to upstream Elk, not a new capability** -- the only patch in the series that is. `bandstr.f90:40` **of the unpatched upstream tree** declares `elm` on the same line as `bc`, under the comment "low precision for band character array saves memory": `real(4), allocatable :: bc(:,:,:,:),elm(:,:)`. The low precision belongs to `bc` alone. `elm` is passed to `genlmirep` and `writeelmirep`, which both declare it `real(8)` (`genlmirep.f90:38`, `writeelmirep.f90:10`), so `genlmirep` writes 8 bytes per element into a 4-byte-per-element allocation -- exactly twice the allocated size. With `lmirep` at its own default of `.true.`, task 22 ((l,m)-resolved band character) aborts with glibc's `corrupted size vs. prev_size` the moment `genlmirep` returns; `lmirep=.false.` and task 21 are unaffected, because `elm` is allocated only inside the `if (lmirep)` branch of the task-22 arm. `dos.f90` is the proof of intent rather than a guess: it makes the same two calls with the same shapes and splits the declarations, `real(4)` for `bc`/`sc` at line 58 and `real(8)` for `elm` at line 60. This patch makes `bandstr.f90` match. Found by running elkpy's own `test_band_character_lm_channel_count` against Elk 11.0.2 -- a heap overrun is invisible until the allocator happens to notice, so nothing but running it would have shown this |
...
- 0025 is the sixth patch to touch `elk.f90`'s dispatch and `Makefile`'s
  `SRC_ELKPY` (with 0001/0002/0003/0011/0012/0024). Its own new file calls
  upstream `gengkvec`, `match`, `eveqnfv` and `eveqnsv` unmodified -- the same
  set 0012 depends on, minus `plotpt2d`/`wfirsv` -- so an upstream signature
  change to one of those is the realistic breakage mode rather than a
  patch-application conflict. Like 0012 it writes only exported ingredients
  (the two plane Gram matrices and the eigenvalues); the trace and the
  broadening are in `src/elkpy/parsers/transport.py`. The one hazard that is
  its own is physical rather than structural: the vacuum Gram matrices come
  from the interstitial plane-wave sum, whose reach is set by `rgkmax`, so a
  pocket at large in-plane momentum falls onto a height-independent floor
  (~1e-6 on NbSe2 at `rgkmax=7`, past ~3.5 A) while still returning plausible
  numbers. Sweep the tip height and check both pockets are straight on a log
  plot before quoting a contrast ratio -- `docs/design.md` §35.
- 0026 is the only patch in the series that FIXES upstream Elk rather than
  adding to it, and it is the only one whose hunk should disappear on an
  upstream bump: check whether `bandstr.f90`'s `elm` declaration has been
  corrected before re-applying it. If it has, drop the patch; if the line is
  unchanged, the bug is still there. It touches no elkpy file, no task
  dispatch and no `Makefile`, so it is independent of every other patch here.
- Task numbers 9000-9007 and the `elkpy_`-prefixed block/variable names are
  deliberately in an unused-by-upstream range (`docs/design.md` §8) to
  minimize collision risk on a version bump.
- 0024 is the fifth patch to touch `elk.f90`'s dispatch and `Makefile`'s
  `SRC_ELKPY` (with 0001/0002/0003/0011/0012), and the same re-hooking cost
  applies. Its own new file calls nothing but upstream subroutines, in
  `gndstate.f90`'s own order and with no arguments, so the realistic
  breakage mode on an upstream bump is `gndstate` reordering its own
  initialisation rather than a patch-application conflict -- and the check
  for that is a one-line diff of `gndstate.f90`'s `trdstate` branch and the
  head of its SCF loop against the sequence `elkpy_initstate.f90` documents.
  It shares this with 0011: the routine deliberately mirrors an upstream
  code path rather than calling it, because the path in question is not a
  subroutine.
