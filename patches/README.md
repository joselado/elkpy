...
| [0019](0019-densityk-export.patch) | — | `src/elkpy_eigenstates.f90` (elkpy's own file, added by 0003) | New `elkpy_denskexport` subroutine + a `DENSITYK` arm in the query loop | `DENSITYK` query — exactly what `rhomagv` feeds to `rhomagk`, for the ground state's OWN k-set: `wkpt`, `vkl`, `occsv`, `ngk`, `igkig`, and `evecfv` read back through `getevecfv` the same way `rhomagv` reads it, plus the **coarse** interstitial grid `ngdgc`/`ngtc`/`igfc` the density is accumulated on. Not part of `LAPW` (one k-point, the caller's choice) or `GROUNDSTATE` (k-independent): the valence density is a zone sum, and rebuilding the k-set from `ngridk` would differ whenever `reducek` is nonzero, which is the default. Used by `src/elkjax/density.py`, which reproduces Elk's interstitial valence density to 9e-16 relative on an unreduced mesh (`docs/jax_port_phase2.md` §2h) |
