...
!   constructed from a sum of $n$ sinusoidal waves, each modulated with a
...
!   Eight real numbers have to be specified for each pulse, namely the vector
!   amplitude ${\bf A}_0$, frequency $\omega$, phase $\phi$, chirp rate
!   $r_{\rm c}$, peak time $t_0$, full-width at half-maximum
!   $d=2\sqrt{2\ln 2}\sigma$.
!
! !REVISION HISTORY:
...
real(8) av(3),s0,sv(3)
real(8) t1,t2,t3,t4
! conversion factor of power density to W/cm²
...
! spin-dependent vector potential
    if (tafspt) then
      do j=1,3
        afspt(1:3,j,its)=afspt(1:3,j,its)+av(1:3)*sv(j)
      end do
      av(1:3)=s0*av(1:3)
    end if
    afieldt(1:3,its)=afieldt(1:3,its)+av(1:3)
  end do
end do
!---------------!
!     ramps     !
...
! loop over time steps
  do its=1,ntimes
    ...
    if (t >= t0) then
      if (tafspt) then
        do j=1,3
          afspt(1:3,j,its)=afspt(1:3,j,its)+av0(1:3)*sv(j)
        end do
      end if
      afieldt(1:3,its)=afieldt(1:3,its)+av(1:3)
    end if
...
close(50)
! write the spin-dependent vector potential to AFSPT.OUT
if (tafspt) then
