...
if (mp_mpi) then
  write(*,*)
  write(*,'("Number of MPI processes : ",I0)') np_mpi
  write(*,'("Number of OpenMP threads per MPI process : ",I0)') maxthd
  write(*,'("Total number of threads (MPI x OpenMP) : ",I0)') np_mpi*maxthd
  write(*,'("Maximum OpenMP nesting level : ",I0)') maxlvl
  write(*,'("Number of OpenMP threads at first nesting level : ",I0)') maxthd1
  write(*,'("Number of MKL threads : ",I0)') maxthdmkl
...
! check if task can be run with MPI
  if (lp_mpi > 0) then
    if (all(task /= [0,1,2,3,5,15,16,28,29,61,62,63,110,120,121,125,135,136, &
     ...
     601,610,620,630,640,670,680,700,701,720,725])) then
      write(*,'("Info(elk): MPI process ",I0," idle for task ",I0)') lp_mpi,task
      goto 20
    end if
  end if
...
!BOI
! !TITLE: {\huge{\sc The Elk Code Manual}}\\ \Large{\sc Version 11.0.2}\\ \vskip 20pt \includegraphics[height=1cm]{elk_silhouette.pdf}
...
!   Claudia Draxl, Clas Persson, Fredrik Bultmark, Christian Brouder, Rickard
...
!   Antonio Sanna, Tyrel McQueen, Tim Baldsiefen, Marty Blaber, Anton
!   Filanovich, Torbj\"{o}rn Bj\"{o}rkman, Martin Stankovski, Jerzy Goraus,
!   Markus Meinert, Daniel Rohr, Vladimir Nazarov, Kevin Krieger, Pink Floyd,
!   Arkardy Davydov, Florian Eich, Aldo Romero Castro, Koichi Kitahara, James
!   Glasbrenner, Konrad Bussmann, Igor Mazin, Matthieu Verstraete, David
...
!   Fischer, Wenhan Chen, Jyoti Krishna and Sebastian Kalh\"{o}fer. Special
...
!   space, usually accompanied with a `segmentation fault' error. To avoid this,
!   increase the main and OpenMP stack sizes with (for example)
!   \begin{verbatim}
!    export OMP_STACKSIZE=512M
!    ulimit -Ss 524288
!   \end{verbatim}
!   before running the code. Alternatively, use the script file {\tt elk.sh}
!   found in the {\tt elk/} directory which increases the stack space and sets
!   additional OpenMP environment variables
!
...
!   character.
!
!   \subsubsection{Stopping the code}
!   To cleanly stop an Elk run simply create the empty file {\tt STOP} in the
!   running directory. This can be done with
!   \begin{verbatim}
!    touch STOP
!   \end{verbatim}
!   The code can be completely restarted with a changed {\tt elk.in} by creating
!   the empty file {\tt RESTART} after creating the {\tt STOP} file. This is
!   particularly convenient when running on a shared computer because the input
!   file can be changed without having to resubmit the job.
!
...
!
!   \block{actype}{
!   {\tt actype} & analytic continuation type & integer & 10}
!   This defines the type of numerical analytic continuation to be used in $GW$
!   calculations for continuing functions, defined on the Matsubara frequencies,
!   to the real axis. Currently implemented are:
!   \vskip 6pt
!   \begin{tabularx}{\textwidth}[h]{lX}
!    1  & Simple pole model (see routines {\tt acpole} and {\tt zfpole}) \\
!    10 & Stabilised Pad\'{e} approximant (see routines {\tt pade} and
!         {\tt pades})
!   \end{tabularx}
!   \vskip 6pt
!
...
!
!   \block{autodlefe}{
!   {\tt autodlefe} & {\tt .true.} if the difference between the fixed
!    linearisation energies and Fermi energy should be found automatically &
!    logical & {\tt .true.}}
!   When this variable is {\tt .true.} then {\tt dlefe} is set to the first
!   energy moment of eigenvalues below the Fermi energy down to {\tt esccut}.
!   This is used only when {\tt autolinengy} is also {\tt .true.}. See the
!   routine {\tt finddlefe}.
!
!   \block{autokpt}{
...
!
!   \block{lprojw90}{
!   {\tt is(i)} & species number & integer & - \\
!   {\tt l(j,i)} & angular momentum values & integer & -100}
!   Sets the angular momentum values of the Wannier90 projectors for a list of
!   species. The list should be terminated with a blank line. Note that the
!   angular part of the projectors are the cubic harmonics for $l=0\ldots 3$.
!   See {\tt projw90} and the routine {\tt projkw90}.
!
!   \block{lradstp}{
...
!
!   \block{mefvs}{
!   {\tt mefvs} & parameter determining the size of the subspace used for the
!    first-variational eigenvalue problem & integer & $-1$}
!   When ${\tt mefvs}\ne -1$ then the first-variational eigenvalue problem is
!   solved in a subspace of the APW+local-orbital basis corresponding to the
!   lowest $m$ diagonal elements of the Hamiltonian. When ${\tt mefvs}\ge 0$
!   then $m=\min(\max({\tt mefvs},{\tt nstfv}),n_{\bf k})$, where $n_{\bf k}$ is
!   the number of APW+l.o.\ basis functions at $k$-point {\bf k}. If
!   ${\tt mefvs}<0$ then $m=n_{\bf k}/|{\tt mefvs}|$. Compute time can be
!   lowered by decreasing $m$ however this will also reduce accuracy.
!
!   \block{mixsave}{
...
!   3 & Modified Broyden mixing, see G. P. Srivastava, {\it J. Phys. A: Math.
!       Gen.} {\bf 17}, L317 (1984) and D. D. Johnson, {\it Phys. Rev. B}
!       {\bf 38}, 12807 (1988)
!   \end{tabularx}
!
!   \block{mixsdb}{
...
!
!   \block{projw90}{
!   {\tt projw90} & {\tt .true.} if the $s$, $p$, $d$ and $f$ projectors should
!    be calculated & logical & {\tt .false.}}
!   When this is {\tt .true.}, muffin-tin projectors in the cubic harmonic basis
!   are used to calculate the Wannier90 matrix $A_{mn}$. Otherwise complex
!   random numbers are used. See also {\tt lprojw90}.
!
!   \block{pulse}{
...
!    spectral function). \\
!   630 & Writes the $GW$ Fermi energy to {\tt GWEFERMI.OUT} \\
...
!   720/5 & Ultra long-range band structure and spectral function. Task 720 is
!    for the central $k$-point (i.e. $\kappa=0$), while task 725 averages over
!    all $\kappa$-points.\\
...
!    treated as diagonal & logical & {\tt .false.}}
