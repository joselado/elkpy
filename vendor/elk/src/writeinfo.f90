...
do is=1,nspecies
  write(fnum,*)
  write(fnum,'("Species : ",I0," (",A,")")') is,trim(spsymb(is))
  ...
  do ia=1,natoms(is)
    write(fnum,'("  ",I0," : ",3F12.8,"  ",3F12.8)') ia,atposl(:,ia,is), &
     bfcmt(:,ia,is)
  end do
end do
write(fnum,*)
...
if ((abs(fsmtype) == 2).or.(abs(fsmtype) == 3)) then
  ...
  do is=1,nspecies
    write(fnum,'("  species : ",I0," (",A,")")') is,trim(spsymb(is))
    do ia=1,natoms(is)
      write(fnum,'("   ",I0,3G18.10)') ia,mommtfix(:,ia,is)
    end do
  end do
end if
if (tssxc) then
...
case(1:)
  if (isgkmax <= nspecies) then
    write(fnum,'(" using radius of species ",I0," (",A,")")') isgkmax, &
     trim(spsymb(isgkmax))
  end if
end select
...
write(fnum,*)
write(fnum,'("Number of empty states         : ",I6)') nempty
write(fnum,'("Total number of valence states : ",I6)') nstsv
write(fnum,'("Total number of core states    : ",I6)') nstcr
write(fnum,*)
if (lorbcnd) then
  write(fnum,'("Conduction state local-orbitals added automatically")')
end if
write(fnum,'("Total number of local-orbitals : ",I6)') nlotot
if (mefvs /= -1) then
  write(fnum,*)
  write(fnum,'("First-variational eigenvalue equation solved in subspace")')
  write(fnum,'(" subspace size parameter, mefvs : ",I6)') mefvs
end if
write(fnum,*)
if (task == 5) then
...
if (dftu /= 0) then
  ...
  do i=1,ndftu
    ...
    l=isldu(2,i)
    if (inpdftu == 1) then
      write(fnum,'(" species : ",I0," (",A,")",", l = ",I0,", U = ",F12.8, &
       &", J = ",F12.8)') is,trim(spsymb(is)),l,ujdu(1,i),ujdu(2,i)
    else if (inpdftu == 2) then
      write(fnum,'(" species : ",I0," (",A,")",", l = ",I0)') is, &
       trim(spsymb(is)),l
