...
if (allocated(tfeqat)) deallocate(tfeqat)
allocate(tfeqat(natmmax,nspecies))
...
! flag the first equivalent atom for each species
do is=1,nspecies
  do ia=1,natoms(is)
    ja=findloc(eqatoms(1:ia,ia,is),.true.,1)
    tfeqat(ia,is)=(ia == ja)
  end do
end do
if (tshift) then
