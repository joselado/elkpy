...
! !DESCRIPTION:
!   Solves the generalised eigenvalue equation,
!   $$ (H-\varepsilon O)|v\rangle=0, $$
...
! no inversion symmetry: use complex Hermitian matrix eigen solver
  if (mefvs == -1) then
! solve in complete space
    call zhegvxi(nmatp,nstfv,nmatp,h,o,evalfv,nmatmax,evecfv)
  else
! solve in subspace
    call zhegvxsi(nmatp,nstfv,mefvs,nmatp,h,o,evalfv,nmatmax,evecfv)
  end if
end if
call timesec(ts1)
!$OMP ATOMIC
