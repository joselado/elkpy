...
complex(8), allocatable :: kmat(:,:),pmat(:,:,:),a(:,:),b(:,:),c(:,:)
...
!$OMP END DO
deallocate(evecsv,evectv,evecsvt,kmat,pmat,a,b,c)
!$OMP END PARALLEL
call freethd(nthd)
...
if (ntsbackup > 0) then
  if (mod(itimes-1,ntsbackup) == 0) call tdbackup
end if
