...
real(8) gmin,gmax,t1
! allocatable arrays
real(8), allocatable :: wq(:),gmr(:,:,:),gq(:,:),gp(:,:)
complex(8), allocatable :: dq(:,:),ev(:,:),b(:,:)
complex(8), allocatable :: gmq(:,:,:),gmp(:,:)
! initialise universal variables
call init0
call init2
call initph
allocate(wq(nbph),gmr(nbph,nbph,nqptnr),gq(nbph,nqpt),gp(nbph,npp1d))
allocate(dq(nbph,nbph),ev(nbph,nbph),b(nbph,nbph))
allocate(gmq(nbph,nbph,nqpt),gmp(nbph,nbph))
...
deallocate(wq,gmr,gq,gp,dq,ev,b,gmq,gmp)
end subroutine
