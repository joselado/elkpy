...
if (mp_mpi) then
  if (ip0gw == 1) then
    ...
    open(85,file='GWBAND.OUT',form='FORMATTED',action='WRITE',position='APPEND')
  end if
end if
! loop over plot points along path
do ip=ip0gw,npp1d
  if (mp_mpi) then
    write(*,'("Info(gwbandstr): ",I0," of ",I0," plot points")') ip,npp1d
  end if
...
deallocate(vmt,vir,bmt,bir,wr,sf,se)
if (mp_mpi) then
  close(85)
  write(*,*)
