...
| **Occupied projector** $P=\sum_j f_j c_jc_j^\dagger$ | **needs a custom rule** | Gauge-invariant and mathematically differentiable when the window is gapped, *and still returns garbage under JAX's default VJP*, because `eigh`'s rule divides by $(\lambda_i-\lambda_j)$ before the $(f_i-f_j)$ numerator can cancel. Fix: form $K_{ij} = (f_i-f_j)/(\lambda_i-\lambda_j)$ as **one** expression with `jnp.where` and the analytic $f'(\lambda)$ on the near-degenerate block. Measured: error at exact degeneracy $2.08\times10^{-2}\to3.0\times10^{-10}$; jitter reproducibility $159\%\to4.3\times10^{-15}$. **Scope caveat, added after this document was drafted and not yet reconciled with the measurement above:** an independent check finds this claim holds for *smeared* occupations but appears overstated for a **hard integer window with the multiplet fully enclosed**, where the two divergent terms are exact negatives and cancel bitwise — measured agreement with central FD of $1.5\times10^{-9}$ and $7.4\times10^{-10}$ on two of three assemblies of the same spectrum. The third showed $1.3\times10^{-2}$, which is FD noise near a degeneracy rather than an AD error, and that unreliability is exactly why a $6\times6$ toy cannot settle this: FD is not a trustworthy reference here. With Fermi-Dirac smearing the same test does reproduce percent-level error ($2.1$–$5.0\times10^{-2}$), consistent with the $2.08\times10^{-2}$ above. If the distinction holds, the mitigation for an insulator is the one this project already applies to Berry curvature (CLAUDE.md §13): window the whole degenerate group. See `docs/continue_here.md` for the reproduction script and what would settle it. |
