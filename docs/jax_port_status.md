...
| **The compiled step** | whether the port can be pointed at a production cell and mesh at all | **done** (§3d) — one step was 93 s of XLA compile against 208 ms of run, and both loops that caused it were Python: `rschrodint`'s `lax.scan` emitted once per (atom, $\ell$, order) and the whole zone unrolled per k-point. Batched by species and put under `lax.map`/`lax.scan`, compile is 48 s and **independent of the k-mesh** — 86,839 HLO lines at 3, 8 and 16 k-points, against 1,950 lines and 4.5 s of compile per k-point before. Memory is now flat in $n_{\bf k}$ too: the scan rebuilds $H,O$ per k instead of holding all of them (26.8 GiB at production shape). No physical number moves; the acceptance criterion is §3b's fixed point at 1.8e-15/1.0e-9, which is unchanged |
